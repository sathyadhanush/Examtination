...
const saveExams = async (req, res) => {
  try {
    ...
    const { exam_name, exam_code, exam_uuid, noofquestions, timing, is_delete, is_active, exam_type_id, iscurrect  } = result;
    ...
    if (error) {
      ...
      let examsData = await executeQuery(
        ...
        "insert into exams(exam_name, exam_code, exam_uuid, noofquestions, timing, is_delete, is_active, exam_type_id, iscurrect ) values(?,?,?,?,?,?,?,?,?)",
        [exam_name, exam_code, exam_uuid, noofquestions, parseInt(timing), is_delete, is_active, exam_type_id, iscurrect ]
      );
    }
} catch (err) {
...
const updateExams = async (req, res) => {
  let id = req.query.id;
  console.log("id", id);
  const { exam_name, exam_code, exam_uuid, noofquestions, timing, is_delete, is_active, exam_type_id, iscurrect  } = req.body;
  ...
  try {
    ...
    if (examsData.length > 0) {
      ...
      examsData = await executeQuery(
        `update exams set exam_name=?,exam_code=?,exam_uuid=?,noofquestions=?,timing=?,is_delete=?,is_active=?,exam_type_id=?,iscurrect=? where id=${id}`,
        [exam_name, exam_code, exam_uuid, noofquestions, parseInt(timing), is_delete, is_active, exam_type_id, iscurrect]
      );
