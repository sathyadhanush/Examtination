...
const saveEmployee = async (req, res) => {
  try {
    ...
    if (error) {
      ...
      let employeeData = await executeQuery(
        "insert into employees(emp_name,emp_email,emp_address,emp_phone) values(?,?,?,?)",
        [emp_name, emp_email, emp_address, emp_phone]
      );
      employeeData = await executeQuery(
        `select * from employees where emp_id=${employeeData.insertId}`
      );
...
const updateEmployee = async (req, res) => {
  ...
  try {
    ...
    if (employeeData.length > 0) {
      ...
      employeeData = await executeQuery(
        `update employees set emp_name=?,emp_email=?,emp_address=?,emp_phone=? where emp_id=${id}`,
        [emp_name, emp_email, emp_address, emp_phone]
      );
